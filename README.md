...
一款原生 macOS Codex 用量工具。深色额度面板、彩色额度圆环、菜单栏百分比，以及账号历史用量趋势。
...
- **菜单栏**：Codex 图标按剩余额度自下而上填充颜色，旁边显示剩余百分比；点击查看完整额度，右键可打开主面板、刷新或退出。
- **外观模式**：支持跟随 macOS、固定浅色和固定深色；以冷灰、深海军蓝和科技蓝构成统一品牌配色。
...
- **偏好设置**：调整外观、刷新频率和菜单栏显示，按需开启低额度提醒、开机启动。
- 关闭主窗口后，应用继续在菜单栏运行。`⌘Q` 完全退出，`⌘R` 刷新，`⌘,` 打开设置。
- 应用采用单实例保护：即使分别打开项目内和已安装的副本，也只保留一个运行实例，并唤起已有主面板。
...
- `Sources/CodexPulse/App.swift`：菜单栏、弹出详情与主窗口管理。
- 其余 SwiftUI 文件：主面板、图表、设置和详情组件。
